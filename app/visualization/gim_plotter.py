...
def plot_gim_maps(
    ...
    *,
    ...
    for idx in range(len(plot_times)):
        row = idx // ncols
        col = idx % ncols
        items_in_row = min(ncols, len(plot_times) - row * ncols)
        row_offset = ncols - items_in_row
        start_col = row_offset + 2 * col
        spec = grid[row, start_col : start_col + 2]
        axes.append(
            fig.add_subplot(
                spec,
                projection=resolve_map_projection(resolved_projection_name),
            )
        )
